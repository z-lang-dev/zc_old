...
下面的示例展示Z语言已经实现的语法。具体实现可以参看编译器的[测试用例](test.sh)。

#### 最简单的示例

Z语言支持脚本模式的源码，存放成`.zs`格式即可。

Z语言的每一行代码都是一个表达式，最后一行表达式的值就是整个脚本的返回值。
...
Z语言中，用来表示量的东西有4种，我们统称为`值量`（Value），即它是一种“量”，背后代表了一个值。
...
上面展示的是已经实现的Z语言语法示例；接下来展示的是我对Z语言的初步设计，未来会逐步实现，具体参看[开发计划](#开发计划)一节。

**注意**：本节及之后的代码示例，只起到概念展示的作用，暂时并不能真正运行。
...
// 注：main函数是一种特殊情况，它往往是变函数（例如调用println），但考虑到main函数的唯一性、常用性，避免初学者过早接触mut概念，所以main函数默认省略了mut关键字。
